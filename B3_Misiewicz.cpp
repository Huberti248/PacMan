...
std::string toString(float number, int places)
{
    std::string str = std::to_string(number);
    std::string before = str.substr(0, str.find("."));
    std::string after = str.substr(str.find(".") + 1, places);
    return before + "." + after;
}
...
void run()
{
    ...
    SDL_Texture* player2T = IMG_LoadTexture(renderer, "res/player2.png");
    ...
    SDL_Texture* whiteGhost = IMG_LoadTexture(renderer, "res/whiteGhost.png");
...
gameBegin:
    ...
    Text frameTimeTxt;
    frameTimeTxt.dstR.w = 200;
    frameTimeTxt.dstR.h = 15;
    frameTimeTxt.dstR.x = windowWidth - frameTimeTxt.dstR.w;
    frameTimeTxt.dstR.y = 2;
    SDL_Rect leftTeleportR;
    leftTeleportR.x = 11;
    leftTeleportR.y = 139;
    leftTeleportR.w = 46 - leftTeleportR.x + 1;
    leftTeleportR.h = 157 - leftTeleportR.y + 1;
    SDL_Rect rightTeleportR;
    rightTeleportR.x = 190;
    rightTeleportR.y = 140;
    rightTeleportR.w = 224 - rightTeleportR.x + 1;
    rightTeleportR.h = 157 - rightTeleportR.y + 1;
    bool isAbleToEatGhosts = false;
    bool shouldShowWhiteGhosts = true;
    bool shouldShowFirstPlayerTexture = true;
    Clock playerAnimationClock;
    Clock playerAbleToEatGhostsClock;
    Clock ghostAreNotGoingToBeAbleToByEatenClock;
    Clock ghostsMotionClock;
    Clock playerMotionClock;
    Clock globalClock;
    while (running) {
      ...
      // NOTE: Ghosts motion can work like they go to pointRectsForGhostsMovement because the player needs to collect points.
        if (ghostsMotionClock.getElapsedTime() > 100) {
            ghostsMotionClock.restart();
            for (int i = 0; i < ghosts.size(); ++i) {
                Node startNode;
        ...
        for (int i = 0; i < ghosts.size(); ++i) {
            ...
            if (SDL_HasIntersection(&ghosts[i].r, &p.r)) {
        ...
        for (int i = 0; i < pointRects.size(); ++i) {
            if (SDL_HasIntersection(&p.r, &pointRects[i])) {
                if (pointRects[i].x == 62 && pointRects[i].y == 28 || pointRects[i].x == 38 && pointRects[i].y == 268 || pointRects[i].x == 194 && pointRects[i].y == 88 || pointRects[i].x == 186 && pointRects[i].y == 208) {
                    ...
                    playerAbleToEatGhostsClock.restart();
                    ghostAreNotGoingToBeAbleToByEatenClock.restart();
                }
        ...
        if (playerAbleToEatGhostsClock.getElapsedTime() > 5000) {
            isAbleToEatGhosts = false;
        }
        if (SDL_HasIntersection(&p.r, &leftTeleportR)) {
            p.r.x = rightTeleportR.x - p.r.w - 1;
        }
        if (SDL_HasIntersection(&p.r, &rightTeleportR)) {
            p.r.x = leftTeleportR.x + leftTeleportR.w + 1;
        }
        if (keys[SDL_SCANCODE_A] || keys[SDL_SCANCODE_LEFT] || keys[SDL_SCANCODE_D] || keys[SDL_SCANCODE_RIGHT] || keys[SDL_SCANCODE_W] || keys[SDL_SCANCODE_UP] || keys[SDL_SCANCODE_S] || keys[SDL_SCANCODE_DOWN]) {
            if (playerAnimationClock.getElapsedTime() > 100) {
                playerAnimationClock.restart();
                shouldShowFirstPlayerTexture = !shouldShowFirstPlayerTexture;
            }
        }
        frameTimeTxt.setText(renderer, robotoF, toString(deltaTime, 2) + " ms | " + toString(1000 / deltaTime, 2) + " FPS");
        ...
        for (int i = 0; i < ghosts.size(); ++i) {
            if (isAbleToEatGhosts) {
                if (playerAbleToEatGhostsClock.getElapsedTime() > 3000) {
                    if (ghostAreNotGoingToBeAbleToByEatenClock.getElapsedTime() > 100) {
                        ghostAreNotGoingToBeAbleToByEatenClock.restart();
                        shouldShowWhiteGhosts = !shouldShowWhiteGhosts;
                    }
                    if (shouldShowWhiteGhosts) {
                        SDL_RenderCopy(renderer, whiteGhost, 0, &ghosts[i].r);
                    }
                    else {
                        if (i == 0) {
